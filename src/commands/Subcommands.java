...
public class Subcommands {
	...
	public void wsjoin(Player player, String arenaName){
		...
		if(((WinterSlashMain) WinterSlashMain.getInstance()).isInventoryEmpty(player)){
			player.sendMessage(ChatColor.YELLOW + "Please empty your inventory!");
			return;
		}
		
		if(WinterSlashController.getManager().getArena(arenaName).getPlayers().contains(player.getName())){
		...
		player.sendMessage(ChatColor.YELLOW + "You have been put on the games waiting list.");
		 ...
		 config.set("PlayerData." + player.getName() + ".X", player.getLocation().getBlockX());
         config.set("PlayerData." + player.getName() + ".Y", player.getLocation().getBlockY());
         config.set("PlayerData." + player.getName() + ".Z", player.getLocation().getBlockZ());
         config.set("PlayerData." + player.getName() + ".Yaw", player.getLocation().getYaw());
         config.set("PlayerData." + player.getName() + ".Pitch", player.getLocation().getPitch());
         config.set("PlayerData." + player.getName() + ".World", Bukkit.getName());
         config.set("PlayerData." + player.getName() + ".World", Bukkit.getName());
